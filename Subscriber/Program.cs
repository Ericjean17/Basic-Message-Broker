...
static async Task<List<int>> GetMessagesAsync(HttpClient httpClient)
{
  ...
  try
  {
    newMessages = await httpClient.GetFromJsonAsync<List<MessageReadDto>>("http://localhost:5069/api/subscriptions/2/messages");
  }
  catch
  {
    return ackIds;
...
{
  var response = await httpClient.PostAsJsonAsync("http://localhost:5069/api/subscriptions/2/messages", ackIds);
